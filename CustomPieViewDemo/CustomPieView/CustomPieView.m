...
// 饼状图 array
@property (nonatomic,strong) NSMutableArray *pieShapeLayerArray;

// 各个部分的coverLayer
@property (nonatomic,strong) NSMutableArray *segmentCoverLayerArray;

// 各个部分的path
@property (nonatomic,strong) NSMutableArray *segmentPathArray;

// 半径
@property (nonatomic) CGFloat pieR;

// 圆心
@property (nonatomic) CGPoint pieCenter;
...
- (void)updatePieView
{
    ...
    // 饼状图 array self.pieShapeLayerArray
    for (CAShapeLayer *layer in self.pieShapeLayerArray) {
    ...
    [self.pieShapeLayerArray removeAllObjects];
    [self.segmentPathArray removeAllObjects]; // 各个部分的path
    [self.segmentCoverLayerArray removeAllObjects]; // 各个部分的coverLayer
...
{
    self.pieShapeLayerArray = [NSMutableArray array]; // 饼状图 array
    self.segmentPathArray = [NSMutableArray array]; // 各个部分的path
    self.segmentCoverLayerArray = [NSMutableArray array]; // 各个部分的coverLayer
    self.colorPointArray = [NSMutableArray array]; // 小圆点颜色数组
...
{
    // 半径
    self.pieR = [self preferGetUserSettingRadiusValue];
    // 圆心
    [self loadPieCenter];
    
    if (self.centerXPosition > 0  ) {
        self.pieCenter = CGPointMake(self.centerXPosition, self.pieCenter.y);
    }
    if (self.centerYPosition > 0 ) {
        self.pieCenter = CGPointMake(self.pieCenter.x, self.centerYPosition);
    }
    ...
    CGFloat innerWhiteRadius = [self preferGetUserSetInnerRadiusValue:self.innerCircleR withDefaultValue:self.pieR/3];
    ...
    for ( int i = 0 ;i < self.segmentDataArray.count;i++) {
        ...
        [path addArcWithCenter:self.pieCenter radius:self.pieR startAngle:currentRadian endAngle:endAngle clockwise:YES];
        [path addArcWithCenter:self.pieCenter radius:innerWhiteRadius startAngle:endAngle endAngle:currentRadian clockwise:NO];
        // 添加到圆心直线
        [path addLineToPoint:self.pieCenter];
        // 路径闭合
        [path closePath];
        
        // 当前shapeLayer的遮罩
        UIBezierPath *coverPath = [UIBezierPath bezierPathWithArcCenter:self.pieCenter radius:self.pieR/2+offset startAngle:currentRadian endAngle:endAngle clockwise:YES];
        [self.segmentPathArray addObject:coverPath];
        
        // 初始化Layer
        CustomShapeLayer *radiusLayer = [CustomShapeLayer layer];
        // 设置layer的路径
        radiusLayer.centerPoint = self.pieCenter;
        radiusLayer.startAngle = currentRadian;
        radiusLayer.endAngle = endAngle;
        radiusLayer.radius = self.pieR;
        ...
        [self.pieShapeLayerArray addObject:radiusLayer];
        
        [self.layer addSublayer:radiusLayer];
    }
    
    
    for (int i = 0;i < self.segmentPathArray.count ;i++) {
        UIBezierPath *path = self.segmentPathArray[i];
        CAShapeLayer *originLayer = self.pieShapeLayerArray[i];
        
        CAShapeLayer *layer = [CAShapeLayer layer];
        layer.lineWidth = self.pieR+offset*2;
        layer.strokeStart = 0;
        layer.strokeEnd = 0;
        ...
        [self.segmentCoverLayerArray addObject:layer];
    }
    
    UIBezierPath *whitePath = [UIBezierPath bezierPathWithArcCenter:self.pieCenter radius:innerWhiteRadius startAngle:0 endAngle:M_PI*2 clockwise:YES];
    self.whiteLayer = [CAShapeLayer layer];
    self.whiteLayer.path = whitePath.CGPath;
    self.whiteLayer.fillColor = innerColor.CGColor;
    [self.layer addSublayer:self.whiteLayer];
}

- (void)doSegmentAnimation
{
    for (CAShapeLayer *layer in self.segmentCoverLayerArray) {
...
- (void)loadPieView
{
    ...
    // 圆弧 默认最右端为0，YES时为顺时针。NO时为逆时针。
    [innerPath addArcWithCenter:self.pieCenter radius:self.pieR/2+offset startAngle:-M_PI/2 endAngle:M_PI*3/2 clockwise:YES];
    
    // 初始化Layer
    self.coverCircleLayer = [CAShapeLayer layer];
    // 设置layer的路径
    self.coverCircleLayer.lineWidth = self.pieR+offset*2;
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    for (CustomShapeLayer *shapeLayer in self.pieShapeLayerArray) {
        ...
        if (CGPathContainsPoint(shapeLayer.path, 0, touchPoint, YES)) {
            ...
            NSInteger index = [self.pieShapeLayerArray indexOfObject:shapeLayer];
            // 执行block并开始右侧小圆点动画
...
- (void)loadPieCenter
{
    ...
    CGFloat viewWidth = self.bounds.size.width;
    // 圆心
    self.pieCenter = CGPointMake(viewWidth/2, viewHeight/2);
    ...
    switch (self.centerType) {
        ...
        {
            // 圆心
            self.pieCenter = CGPointMake(self.pieR, self.pieR);
        }
            break;
        case PieCenterTypeTopMiddle:
        {
            // 圆心
            self.pieCenter = CGPointMake(viewWidth/2, self.pieR);
        }
            break;
        case PieCenterTypeTopRight:
        {
            // 圆心
            self.pieCenter = CGPointMake(viewWidth-self.pieR, self.pieR);
        }
            break;
        case PieCenterTypeMiddleLeft:
        {
            // 圆心
            self.pieCenter = CGPointMake(self.pieR, viewHeight/2);
        }
            break;
        case PieCenterTypeMiddleRight:
        {
            // 圆心
            self.pieCenter = CGPointMake(viewWidth-self.pieR, viewHeight/2);
        }
            break;
        case PieCenterTypeBottomLeft:
        {
            // 圆心
            self.pieCenter = CGPointMake(self.pieR, viewHeight-self.pieR);
        }
            break;
        case PieCenterTypeBottomMiddle:
        {
            // 圆心
            self.pieCenter = CGPointMake(viewWidth/2, viewHeight-self.pieR);
        }
            break;
        case PieCenterTypeBottomRight:
        {
            // 圆心
            self.pieCenter = CGPointMake(viewWidth-self.pieR, viewHeight-self.pieR);
        }
            break;
